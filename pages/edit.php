...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tahun < 1800 || $tahun > (int)date('Y')) $errors['tahun'] = 'Tahun tidak valid.';
    if (!in_array($kategori, $kategori_list))      $errors['kategori'] = 'Pilih kategori yang valid.';
    if ($stok < 0)         $errors['stok']      = 'Stok tidak boleh negatif.';

    if (empty($errors)) {
        $upd = $conn->prepare(
            "UPDATE buku SET judul=?, pengarang=?, penerbit=?, tahun_terbit=?, kategori=?, stok=? WHERE id=?"
        );
  ...
  <form method="POST" action="edit.php">
    ...
    <div class="form-grid">
      <div class="form-group full">
        ...
        <input type="text" id="judul" name="judul" value="<?= htmlspecialchars($buku['judul']) ?>">
        <?php if (isset($errors['judul'])): ?><small style="color:var(--danger)"><?= $errors['judul'] ?></small><?php endif; ?>
      </div>
      <div class="form-group">
        <label for="pengarang">Pengarang</label>
        <input type="text" id="pengarang" name="pengarang" value="<?= htmlspecialchars($buku['pengarang']) ?>">
        <?php if (isset($errors['pengarang'])): ?><small style="color:var(--danger)"><?= $errors['pengarang'] ?></small><?php endif; ?>
      </div>
      <div class="form-group">
        <label for="penerbit">Penerbit</label>
        <input type="text" id="penerbit" name="penerbit" value="<?= htmlspecialchars($buku['penerbit']) ?>">
      ...
      <div class="form-group">
        ...
        <input type="number" id="tahun_terbit" name="tahun_terbit"
               value="<?= htmlspecialchars($buku['tahun_terbit']) ?>" min="1800" max="<?= date('Y') ?>">
      ...
      <div class="form-group">
        ...
        <input type="number" id="stok" name="stok" value="<?= htmlspecialchars($buku['stok']) ?>" min="0">
